...
def read_all_test_data() -> pd.DataFrame:
    for test_file_date in ['2025-01-01', '2025-01-08', '2025-01-15', '2025-01-22', '2025-02-05', '2025-02-12', '2025-02-19', '2025-02-26', '2025-02-05', '2025-02-12', '2025-02-19', '2025-02-26']:
        ...
        if 'test_data' not in locals():
            ...
            test_data = pd.concat([test_data, test_data_temp], ignore_index=True)
...
def fix_lag_stacks(df: pd.DataFrame):
    ...
    for var, (lo, hi) in LAG_VALID.items():
        ...
        #print(var, block.shape, valid.shape)
        #sys.exit(1)
...
def remove_fake_zero_sites(df: pd.DataFrame) -> pd.DataFrame:
    df.loc[df['station_name'] == '0', 'station_name'] = 0
    df = df.loc[df.station_name!=0] 
...
def input_verification(df: pd.DataFrame, dataset_tag: str, pic_dir: str):
    # temp_avg is only used for value checking
    df['temp_avg'] = (df['air_temperature_tmmx'] + df['air_temperature_tmmn'])/2
    df.loc[df['temp_avg'] == 0, 'temp_avg'] = np.nan

    plot_var_list = ['lat', 'lon', 'Elevation', 'Aspect', 'Curvature', 'Eastness', 'Northness', 'temp_avg', 'mean_vapor_pressure_deficit', 'potential_evapotranspiration', 'precipitation_amount', 'relative_humidity_rmax', 'wind_speed', 'day_of_year', 'water_year', 'fsca', 'swe_value']
    plot_label_list = ['Lat', 'Lon', 'ELV', 'Aspect', 'Curvature', 'Eastness', 'Northness', 'TEMP', 'VPD', 'Evapo', 'PRECIP', 'RH', 'WS', 'DOY', 'Water_year', 'Fsca', 'SWE']

    print(f'---- {dataset_tag} set')

    from eval_util import input_density_plot
    for var, label in zip(plot_var_list, plot_label_list):
        print(label, 'Min:', df[var].min(), 'Max:', df[var].max())
        input_density_plot(df[var], label, dataset_tag, pic_dir)
...
def main() -> None:
    ...
    ap.add_argument("--test_date", default="2025-03-26")
    ap.add_argument("--out_dir",
                    default="/groups/ESS/whung/swe_gnn/data")
    ...
    ap.add_argument("--no_fake", action="store_true",
                    help="remove fake zero-SWE sites from train set")
    args = ap.parse_args()
    #args.no_fake = True
    args.test_only = True
    t0 = time.time()
    ...
    if args.no_fake:
        INTENDED = (BASE_COLS
            + [f"{v}_{i}" for i in range(1, 8) for v in LAG_VARS]
            + ["water_year", "snodas_mask", "station_name"])
    else:
        INTENDED = (BASE_COLS
            + [f"{v}_{i}" for i in range(1, 8) for v in LAG_VARS]
            + ["water_year", "snodas_mask"])
    ...
    train_m = None
    if args.test_only:
        with open(f"{args.out_dir}/scaler.pkl", "rb") as f:
            saved = pickle.load(f)
        ...
        if train_absmax is None:  # older pickle: derive from the saved graph
            tr_graph = torch.load(f"{args.out_dir}/gnn_training_data.pt",
        ...
        if args.no_fake:
            train = remove_fake_zero_sites(train)
            select.remove("station_name")
    ...
    #test = read_all_test_data()  # for input verification
    ...
    #input_verification(train_m, 'Train', '/groups/ESS/whung/swe_gnn/data/input_density')
    #input_verification(test_m, 'Test', '/groups/ESS/whung/swe_gnn/data/input_density')
    #exit()
    ...
    if not args.test_only:
        if args.no_fake:
            with open(f"{args.out_dir}/scaler_nofake.pkl", "wb") as f:
            ...
            torch.save(make_graph(train_m, x_train, is_test=False),
                    f"{args.out_dir}/gnn_training_data_nofake.pt")
        else:
            with open(f"{args.out_dir}/scaler.pkl", "wb") as f:
                pickle.dump({"scaler": scaler, "columns": final_cols,
                            "train_absmax": train_absmax}, f)
            print("\nbuilding training graph...")
            torch.save(make_graph(train_m, x_train, is_test=False),
                    f"{args.out_dir}/gnn_training_data.pt")
    #print("building test graph...")
    #torch.save(make_graph(test_m, x_test, is_test=True),
    #           f"{args.out_dir}/gnn_testing_data_{args.test_date}.pt")
